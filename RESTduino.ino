...
void setup()
{
  ...
  // Tip from chatgpt - give the ethernet a bit to connect/reset.
  delay(3000);
  ...
  if (client) {
    ...
    while (client.connected()) {
      if (client.available()) {
        ...
        //  fill url the buffer
        if(c != '\n' && c != '\r' && index < BUFSIZE - 1){ // leave space for '\0'
            clientline[index++] = c;
            continue;
        }
        clientline[index] = '\0';  // Ensure null termination
        ...
        if(pin != NULL){
          if(value != NULL){
            ...
            if (pin[0] == 'D' || pin[0] == 'd') {
              selectedPin = atoi(pin + 1);  // skip 'D'
            } else if (pin[0] == 'A' || pin[0] == 'a') {
              selectedPin = analogInputToDigitalPin(atoi(pin + 1));  // optional: remap analog pins if needed
            }
            #if DEBUG
...
#endif
#if DEBUG
  Serial.print("analogWrite to pin ");
